...
class Trainer(Trainable):
    ...
    def __init__(self, config=None, env=None, logger_creator=None):
        ...
        # add tfboard
        if 'tfboard_path' in config:
            self.tfboard_path = config['tfboard_path']
        else:
            self.tfboard_path = None
        del config['tfboard_path']
        if self.tfboard_path is not None:
            self.logger = Logger(self.tfboard_path)
            self.num_iter = 0
    ...
    def train(self):
        ...
        for _ in range(1 + MAX_WORKER_FAILURE_RETRIES):
            try:
                ...
                break
        # tfboard
        # {'episode_reward': [-378.0, -265.0, -412.0], 'episode_lengths': [41, 25, 44], 'policy_schedule_reward': [-290.0, -178.0, -317.0], 'policy_plan_reward': [-88.0, -87.0, -95.0]}
        avg_rew = sum(result['hist_stats']['episode_reward']) / \
                  len(result['hist_stats']['episode_lengths'])
        self.logger.scalar_summary('ALL', avg_rew, self.num_iter+1)
        self.num_iter += 1
