...
import ray
from multiagent.env_logistics import MultiAgentEnv
...
GP_len = 5 #the length of goal period
GP_num = int(300/GP_len) #the number of goal periods


env_config = {
    'scenario_name': 'logistics.py',
    'GP_len': GP_len,
    'GP_num': GP_num,
    'coordinator_len': 5, #1+coordinator_len is the number of global states collected in a goal period for the RGD
...
ppo_trainer = PPOTrainer(
    ...
    config={
        ...
        "clip_param": 0.05,
        "vf_clip_param": 10,
        "tfboard_path": BASE_PATH + 'logistics_GP%i'%GP_len,
        "multiagent": {
            "policies": {
            ...
            "policy_mapping_fn": lambda
                x: "coordinator2" if x == -3 else "coordinator" if x == -2 else "leader" if x == -1 else "agent1" if x == 0 else "agent2" if x == 1 else "agent3" if x == 2 else "agent4" if x == 3 else "agent5",
            "policies_to_train": ["coordinator2", "coordinator", "leader", "agent1", "agent2", "agent3", "agent4", "agent5"],
        },
        "monitor": True,
        "model": {
...
for i in range(num_of_iter):
    ...
    if (i + 1) % checkpoint_iter == 0:
        ppo_trainer.save(BASE_PATH + "save/logistics_checkpoint_" + str(lr) + '_' + str(i + 1))
